...
vault_unseal() {
    ...
    # zsh-массивы 1-индексированы; используем $() + разбивку по строкам
    local keys=("${(@f)$(jq -r '.unsealKeys[:3][]' "$secrets_file")}")
    local i
    for (( i = 1; i <= ${#keys[@]}; i++ )); do
        echo "🔑 Используем ключ $i/3..."
        if ! vault operator unseal "${keys[i]}"; then
            echo "❌ Ошибка при использовании ключа $i"
            return 1
        fi
    done
